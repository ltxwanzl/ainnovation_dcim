...
class HomeView(View):
    ...
    def get(self, request):
        ...
        def build_stats():
            ...
            dcim = (
                ("dcim.view_rack", "机架", Rack.objects.restrict(request.user, 'view').count),
            ...
            circuits = (
                ("circuits.view_provider", "提供商", Provider.objects.restrict(request.user, 'view').count),
                ("circuits.view_circuit", "电路", Circuit.objects.restrict(request.user, 'view').count),
            )
            virtualization = (
            ...
            connections = (
                ("dcim.view_cable", "电缆", Cable.objects.restrict(request.user, 'view').count),
                ("dcim.view_consoleport", "控制台", connected_consoleports.count),
                ("dcim.view_interface", "接口", connected_interfaces.count),
                ("dcim.view_powerport", "电源连接", connected_powerports.count),
            )
            power = (
                ("dcim.view_powerpanel", "电源面板", PowerPanel.objects.restrict(request.user, 'view').count),
                ("dcim.view_powerfeed", "电源", PowerFeed.objects.restrict(request.user, 'view').count),
            )
            schedule = (
                ("schedule.view_powerpanel", "电源面板", PowerPanel.objects.restrict(request.user, 'view').count),
                ("schedule.view_powerfeed", "电源", PowerFeed.objects.restrict(request.user, 'view').count),
            )
            ticket = (
                ("ticket.ticket_manage", "工单管理", PowerPanel.objects.restrict(request.user, 'view').count),
            )
            workflow = (
                ("workflow.workflow_manage", "工作流管理", PowerPanel.objects.restrict(request.user, 'view').count),
            )
            sections = (
                ("组织", org, "domain"),
#                 ("IPAM", ipam, "counter"),
                ("虚拟机", virtualization, "monitor"),
                ("库存", dcim, "server"),
                ("连接", connections, "cable-data"),
                ("电路", circuits, "transit-connection-variant"),
                ("电源", power, "flash"),
            )
