...
from users.models import Token, AdminUser, LoonUserRole, LoonDept, LoonRole
...
class AccountBaseService(BaseService):
    ...
    def get_token_by_app_name(cls, app_name: str)->tuple:
        ...
        :return:
        """
        app_token_obj = Token.objects.filter(app_name=app_name, is_deleted=0).first()
    ...
    def get_user_by_username(cls, username: str)->tuple:
        ...
        :return:
        """
        result = AdminUser.objects.filter(username=username, is_deleted=0).first()
        if result:
            return True, result
        else:
    ...
    def get_user_by_user_id(cls, user_id: int)->tuple:
        ...
        :return:
        """
        result = AdminUser.objects.filter(id=user_id, is_deleted=0).first()
        if result:
            return True, result
        else:
    ...
    def get_user_name_list_by_id_list(cls, user_id_list: list)->tuple:
        ...
        :return:
        """
        user_queryset = AdminUser.objects.filter(id__in=user_id_list, is_deleted=0).all()
    ...
    def get_user_role_id_list(cls, username: str)->tuple:
        ...
        :return:
        """
        user_obj = AdminUser.objects.filter(username=username, is_deleted=0).first()
        if not user_obj:
    ...
    def get_role_user_info_by_role_id(cls, role_id: int, search_value: str='', page: int=1, per_page: int =10)->tuple:
        ...
        user_info_queryset = AdminUser.objects.filter(query_params).all()
    ...
    def get_dept_username_list(cls, dept_id: object)->tuple:
        ...
        user_name_list = []
        if sub_dept_id_list:
            user_queryset = AdminUser.objects.filter(dept_id__in=sub_dept_id_list_total).all()
    ...
    def get_role_username_list(cls, role_id: int)->tuple:
        ...
        if not user_id_list:
            return True, []
        username_queryset = AdminUser.objects.filter(id__in=user_id_list).all()
        username_list = []
    ...
    def get_user_list(cls, search_value: str, page: int=1, per_page: int=10)->tuple:
        ...
        if search_value:
            query_params &= Q(username__contains=search_value) | Q(alias__contains=search_value)
        user_objects = AdminUser.objects.filter(query_params)
    ...
    def get_token_list(cls, search_value: str, page: int=1, per_page: int=10)->tuple:
        ...
        token_objects = Token.objects.filter(query_params)
