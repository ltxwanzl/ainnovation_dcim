...
class DeviceType(PrimaryModel):
    ...
    Each DeviceType can have an arbitrary number of comp onent templates assigned to it, which define console, power, and
...
class Platform(OrganizationalModel):
    ...
    napalm_driver = models.CharField(
        max_length=50,
        blank=True,
        verbose_name='NAPALM驱动',
        help_text='The name of the NAPALM driver to use when interacting with devices'
    )
    napalm_args = models.JSONField(
        blank=True,
        null=True,
        verbose_name='NAPALM参数',
